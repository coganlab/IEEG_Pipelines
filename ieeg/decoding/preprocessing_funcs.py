...
def bin_spikes(spike_times, dt, wdw_start, wdw_end):
    ...
    neural_data = np.empty(
        [num_bins, num_neurons])  # Initialize array for binned neural data
...
# %% BIN_OUTPUT
def bin_output(outputs, output_times, dt, wdw_start, wdw_end,
               downsample_factor=1):
    """
    ...
    if downsample_factor != 1:  # Don't downsample if downsample_factor=1
        ...
        downsample_idxs = np.arange(0, output_times.shape[0],
                                    downsample_factor)
        outputs = outputs[downsample_idxs, :]  # Get the downsampled outputs
        output_times = output_times[
            downsample_idxs]  # Get the downsampled output times
    ...
    outputs_binned = np.empty(
        [num_bins, output_dim])  # Initialize matrix of binned outputs
    # Loop through bins, and get the mean outputs in those bins
    for i in range(num_bins):  # Loop through bins
        # Indices to consider the output signal (in the correct time range)
        idxs = np.where((np.squeeze(output_times) >= edges[i]) & (
                    np.squeeze(output_times) < edges[i + 1]))[0]
...
def get_spikes_with_history(neural_data, bins_before, bins_after,
    ...
    num_examples = neural_data.shape[
        0]  # Number of total time bins we have neural data for
    num_neurons = neural_data.shape[1]  # Number of neurons
    surrounding_bins = bins_before + bins_after + bins_current
    # Number of surrounding time bins used for prediction
    X = np.empty([num_examples, surrounding_bins,
                  num_neurons])  # Initialize covariate matrix with NaNs
    X[:] = np.NaN
    # Loop through each time bin, and collect the spikes occurring in
    # surrounding time bins. Note that the first "bins_before" and last
    # "bins_after" rows of X will remain filled with NaNs, since they don't get
    # filled in below. This is because, for example, we cannot collect 10 time
    # bins of spikes before time bin 8
    start_idx = 0
    ...
    for i in range(num_examples - bins_before - bins_after):
        ...
        # and end_idx (which will have length "surrounding_bins")
        end_idx = start_idx + surrounding_bins
        # Put neural data from surrounding bins in X, starting at row
        # "bins_before"
