function [elecFact,timeFact,trialFact] = viz_ktensor_phonotactic_v2(est_factors,channelsPooled,timeEpoch,phonemeTrial,disp)
...
%     axis square
    ...
    fstat = zeros(1,10);
    for iPhon = 1:9
        cvcmdl = fitlm(trialFact,-log(1+phonemeTrial.phonotactic(:,iPhon)'));
        if(cvcmdl.ModelFitVsNullModel.Pvalue<0.05)
            fstat(iPhon) = cvcmdl.ModelFitVsNullModel.Fstat;
        end
    end
    cvcmdl = fitlm(trialFact,phonemeTrial.phonotactic(:,10)');
    if(cvcmdl.ModelFitVsNullModel.Pvalue<0.05)
        fstat(10) = cvcmdl.ModelFitVsNullModel.Fstat;
    end
