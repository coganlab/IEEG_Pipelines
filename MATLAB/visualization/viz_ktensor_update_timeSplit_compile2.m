function [elecFact,timeFact,trialFact] = viz_ktensor_update_timeSplit_compile2(est_factors,chanMap,selectedChannels,timeGammaPerc,timeGammaProd,trigLabelsSort,nDisp,factorChan,factorTime,clusterScore)


figure;

   
    t = tiledlayout(nDisp+1,4,'TileSpacing','compact');
for iCom = 1:nDisp
...
%     t = tiledlayout(1,2,'TileSpacing','compact');
    ...
    plot(ax1,timeGammaPerc,timeFact(iCom,1:round(length(timeFact)/2)),'LineWidth',2,'Color','k');
    ...
    plot(ax2,timeGammaProd,timeFact(iCom,round(length(timeFact)/2)+1:end),'LineWidth',2,'Color','k')
...
%     t = tiledlayout(1,2,'TileSpacing','compact');
    ...
    bgAx.Box = 'off';
    ax1 =  axes(t);
    ax1.Layout.Tile = 4*(nDisp+1)+2;
    plot(ax1,timeGammaPerc,factorTime(:,1:round(length(timeFact)/2)),'LineWidth',2);
    xline(ax1,timeGammaPerc(end),':');
    ax1.Box = 'off';
    xlim(ax1,[timeGammaPerc(1) timeGammaPerc(end)])
    ...
    ax2 =  axes(t);
    ax2.Layout.Tile = 4*(nDisp+1)+3;
    plot(ax2,timeGammaProd,factorTime(:,round(length(timeFact)/2)+1:end),'LineWidth',2)
    xline(ax2,timeGammaProd(1),':');
    ax2.YAxis.Visible = 'off';
    ax2.Box = 'off';
    xlim(ax2,[timeGammaProd(1) timeGammaProd(end)])
    ...
    nexttile; h = boxplot(clusterScore,factorChan,'symbol','');
     set(h,{'linew'},{2})
     xlabel('Clusters');
     set(gca,'FontSize',15);
