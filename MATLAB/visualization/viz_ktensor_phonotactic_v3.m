...
% Link the axes
   ...
   nexttile;
   [p(1,1),tbl] = anova1(trialFact,phonemeTrial.phonemeUnit(:,1)','off');
   fphoneme(1,1) = tbl{2,5};
   [p(1,2),tbl] = anova1(trialFact,phonemeTrial.phonemeUnit(:,2)','off');
    fphoneme(1,2) = tbl{2,5};
   [p(1,3),tbl] = anova1(trialFact,phonemeTrial.phonemeUnit(:,3)','off');
    fphoneme(1,3) = tbl{2,5};
     [p(2,1),tbl] = anova1(trialFact,phonemeTrial.phonemeClass(:,1)','off');
   fphoneme(2,1)= tbl{2,5};
   [p(2,2),tbl] = anova1(trialFact,phonemeTrial.phonemeClass(:,2)','off');
   fphoneme(2,2) = tbl{2,5};
   [p(2,3),tbl] = anova1(trialFact,phonemeTrial.phonemeClass(:,3)','off');
   fphoneme(2,3) = tbl{2,5};
    [p(3,1),tbl] = anova1(trialFact,phonemeTrial.syllableUnit(:,1)','off');
   fphoneme(3,1)= tbl{2,5};
   [p(3,2),tbl] = anova1(trialFact,phonemeTrial.syllableUnit(:,2)','off');
   fphoneme(3,2) = tbl{2,5};
   [p(3,3),tbl] = anova1(trialFact,phonemeTrial.syllableUnit(:,3)','off');
   ...
   b = bar(x, fphoneme');
    ...
    for i = 1:3
        ...
        for j = 1:3
            if(p(i,j)<0.05)
                text(xtips1(j),ytips1(j),'*','HorizontalAlignment','center',...
...
%     end
    ...
    p_blick(2) = cvcmdl.ModelFitVsNullModel.Pvalue;
    fstat_cvc = cvcmdl.ModelFitVsNullModel.Fstat;
...
%     end
    vcvmdl = fitlm(trialFact(vcvIds),phonemeTrial.phonotactic(vcvIds,10)');
     p_blick(3) = vcvmdl.ModelFitVsNullModel.Pvalue;
    fstat_vcv = vcvmdl.ModelFitVsNullModel.Fstat;

    blickMdl = fitlm(trialFact,phonemeTrial.phonotactic(:,10)');
    p_blick(1) = blickMdl.ModelFitVsNullModel.Pvalue;
    fstat = blickMdl.ModelFitVsNullModel.Fstat;
