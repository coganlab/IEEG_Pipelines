function [ieegStructAll, phonemeTrialAll, channelNameAll] = poolChannelWithSmoteTrial(ieegHGStruct, trialInfoStruct)
                ...
                for iGen = 1:numtrials2pad
                    ...
                    switch(length(iPick))
                        ...
                        case 1
                            dataTempGen(:,iGen,:) = nan(size(dataTemp,1),1,size(dataTemp,3));
                        otherwise
                            dataDiff = dataTemp(:,iPick(2),:) - dataTemp(:,iPick(1),:);
                            dataTempGen(:,iGen,:) = dataTemp(:,iPick(1),:)+rand(size(dataDiff)).*dataDiff;                                           
