...
% 
% Input
% path - directory path that contains rhd files
% fileID - file order number
% channel - channel IDs to load
% Example:
% >> clear
% >> path = 'E:\1024 channel\2019-02-06 Human 1024ch IntraOp\';
% >> [amplifier_data,board_adc_data,t_amplifier] = read_Intan_RHD2000_file_path_update(path,3,10:12);
...
if (data_present)
    ...
    amplifier_data = zeros(length(channel), num_amplifier_samples);
    ...
    for i=1:num_data_blocks
        ...
        if (num_amplifier_channels > 0)
          % amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
             amplifier_data_temp = fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
             amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = amplifier_data_temp(channel,:);
        end
