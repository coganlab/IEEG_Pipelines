function [amplifier_data,board_adc_data,t_amplifier,amplifier_channels] = read_Intan_RHD2000_file_path_update_EDIT_ELIM_OVERHEAD(fullPathToRawDataFile,channel)
...
% 
% Input
% fullPathToRawDataFile - full path to file containing raw data
% channel - channel IDs to load
% Example:
% >> clear
...
tic;
%filename = [path,file];
fid = fopen(fullPathToRawDataFile, 'r');

s = dir(fullPathToRawDataFile);
filesize = s.bytes;
...
if (data_present)
    ...
    amplifier_data = zeros(1, num_amplifier_samples);
    ...
    for i=1:num_data_blocks
        ...
        if (num_amplifier_channels > 0)
             fseek(fid,2*(num_samples_per_data_block)*(channel-1),'cof'); %2 because each sample is two bytes
%            amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
             amplifier_data_temp = fread(fid, [num_samples_per_data_block, 1], 'uint16')';
             amplifier_data(amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = amplifier_data_temp;
             fseek(fid,2*num_samples_per_data_block*(num_amplifier_channels-channel),'cof');
        end
...
% 
% % new for version 2.01: move fullPathToRawDataFile info to base workspace
% fullPathToRawDataFile = file;
% move_to_base_workspace(fullPathToRawDataFile);
