...
os.environ['OMP_NUM_THREADS'] = str(
    cpu_count())  # tedana resets thread count on import.
# See https://github.com/ME-ICA/tedana/issues/473 for details
...
class Preprocessing:
    ...
    def set_out_dir(self,
                    output_dir):  # performs the necessary job of creating a
        # dataset_description.json
        if output_dir is None:
            self._output_dir = os.path.join(self._data_dir,
        ...
        if not os.path.isfile(
                os.path.join(self._output_dir, "dataset_description.json")):
            copy(os.path.join(self._data_dir, "dataset_description.json"),
                 os.path.join(self._output_dir, "dataset_description.json"))
            with open(
                    os.path.join(self._output_dir, "dataset_description.json"),
                    'r') as fst:
    ...
    def FuncHandler(self, fileobj, output, suffix):
        ...
        if type(fileobj) == models.BIDSImageFile:  # setting file to temp file before performing operation
            ...
            raise TypeError(
                'file inputs must be either a BIDSImageFile, pathlike string')

        smatch = re.match(
            r"(.*\.nii(?:\.gz))(((?:\[|\{)\d+(?:\.\.\d+|)(?:\]|\})){1,2})",
            fileobj)  # sub-brick parser similar to afni's
        dmatch = re.match(
            r"(.*\.1D)(((?:\[|\{)\d+(?:\.\.\d+|)(?:\]|\})){1,2})",
            fileobj)  # if you want to understand this mess, go here:
        # https://docs.python.org/3/library/re.html
        if smatch:
            fileobj = smatch.group(1).split(".nii.gz")[0] + "_desc-temp.nii.gz"
            afni.TCatSubBrick(in_files=[
                (smatch.group(1), "'{index}'".format(index=smatch.group(2)))],
                              out_file=fileobj).run()
        elif dmatch:
            fileobj = dmatch.group(1).split(".nii.gz")[0] + "_desc-temp.nii.gz"
            afni.Cat(in_files=[dmatch.group(1)],
                     sel="'{index}'".format(index=dmatch.group(2)),
                     out_file=fileobj).run()

        if not os.path.isabs(fileobj) or not os.path.isfile(
                fileobj):  # checking if file exists and address is absolute
            tempfileobj = os.path.abspath(fileobj)
            if not os.path.isfile(
                    tempfileobj) and self._data_dir is not None:
                # checking working directory for file
                tempfileobj = os.path.join(self.BIDS_layout.root, fileobj)
                if not os.path.isfile(
                        tempfileobj) and self._output_dir is not None:
                    # checiking BIDS root directory for file
                    tempfileobj = os.path.join(self._output_dir, fileobj)
                    if not os.path.isfile(
                            tempfileobj):
                        # checking BIDS derivatives derectory for file
                        raise FileNotFoundError(
                            "could not find {filename} in derivatives, working"
                            ", or root directory, check naming and try "
                            "again".format(
                                filename=fileobj))
                    else:
                        fileobj = tempfileobj
        ...
        if output == None and suffix == None:
            ...
            output = fileobj.split('.nii.gz')[
                         0] + "_desc-" + suffix + '.nii.gz'
        elif suffix == None:
            pass
        else:
            print(
                "both suffix and output filename detected as input, using filename given")
    ...
    def skullstrip(self, fileobj=None, out_file=None, args=None, suffix=None):
        ...
        args_in = ""  # add in terminal flags here (ex: "-overwrite") if you
        # want them called with ubiquity accross the whole script any time
        # this command is called. Otherwise add flags the the "args" argument
        # of the command
        if args is not None:
            args_in = args_in + args
        # running skull stripping (change this to change skull stripping
        # program)
        fsl.BET(in_file=fileobj, out_file=out_file, args=args_in,
    ...
    def warp(self, fileobj1=None, fileobj2=None, out_file=None,
             transformation=None, args=None, saved_mat_file=None, suffix=None):
        """
    ...
    def volreg(self, in_file, out_file=None, suffix=None, base=None,
               tshift=False, interpolation="heptic", onedfile=None,
               onedmat=None):
        ...
        if onedmat is True:
            myreg.inputs.oned_matrix_save = out_file.replace("vrA.nii.gz",
                                                             "vrmat.aff12.1D")
    ...
    def tshift(self, in_file=None, out_file=None, suffix=None,
               interp="heptic"):
        ...
        mytshift = afni.TShift(in_file=in_file, interp=interp,
                               outputtype='NIFTI_GZ')
    ...
    def allineate(self, in_file, out_file=None, suffix=None,
                  final="nearestneighbor", mat=None, base=None, args=""):
    ...
    def calc(self, in_file_a, expr, in_file_b=None, in_file_c=None,
             output=None, suffix=None):
        ...
        mycalc = afni.Calc(in_file_a=in_file_a, in_file_b=in_file_b, expr=expr,
                           out_file=output)
...
if __name__ == "__main__":
    ...
    for sub_id in sub_ids:
        ...
        all_fobj = []
        for BIDSFiles in pre.BIDS_layout.get(scope='raw', subject=sub_id,
                                             suffix='bold',
                                             extension='.nii.gz'):
            all_fobj.append(BIDSFiles)

        for BIDSFiles in pre.BIDS_layout.get(scope='raw', extension='.nii.gz',
                                             suffix='T1w',
                                             acquisition='MPRAGE',
                                             subject=sub_id):
            all_fobj.append(BIDSFiles)

        # copying those files to a new derivatives directory so we can mess
        # with them
        for fobj in all_fobj:
            if pre.is_verbose:
                print(
                    "copying {filename} to preprocessing derivatives directory"
                    "".format(
                        filename=fobj.path))
            try:
                ...
                print(
                    "{files} already exists in the preprocessing directory,"
                    " overwriting...".format(files=fobj.filename))
        ...
        filenames = pre.BIDS_layout.get(scope='derivatives', subject=sub_id,
                                        extension='.nii.gz',
                                        return_type="filename",
                                        acquisition="MPRAGE")
        for filename in filenames:
            pre.skullstrip(filename)
            pre.warp(filename, transformation='deoblique',
                     out_file=filename.split('.nii.gz')[0] + "_do.nii.gz")

        # Calculate and save motion and obliquity parameters, despiking
        # first if not disabled, and separately save and mask the base
        # volume assuming only one anatomical image

        for run in pre.BIDS_layout.get(scope='derivatives', subject=sub_id,
                                       extension='.nii.gz', suffix='bold',
                                       target='run', return_type='id'):

            CWD = os.getcwd()
            os.chdir(pre._output_dir)
            if pre.is_verbose:
                print("denoising and saving motion parameters")
            for fobj in pre.BIDS_layout.get(scope='derivatives',
                                            subject=sub_id,
                                            extension='.nii.gz', suffix='bold',
                                            run=run):
                try:  # checking that image is either single echo or the
                    # first of the multi echo series
                    if not fobj.get_entities()['echo'] == "01":
                        continue
                except KeyError:  # handles single echo
                    pass
                for anat in pre.BIDS_layout.get(scope='derivatives',
                                                subject=sub_id,
                                                extension='.nii.gz',
                                                acquisition='MPRAGE'):
                    pre.warp(fileobj2=fobj.path, fileobj1=anat.path,
                             args="-newgrid 1.000000", saved_mat_file=True,
                             transformation='card2oblique',
                             suffix="anat_to_s" + str(
                                 fobj.get_entities()['subject']).zfill(
                                 2) + "r" + str(
                                 fobj.get_entities()['run']).zfill(
                                 2))  # saving transformation matrix for later
                no_echo = re.sub(r"echo-[0-9]{2}_bold", "bold",
                                 fobj.path.replace(".nii.gz",
                                                   "_desc-vrA.nii.gz"))
                pre.despike(fobj.path, out_file=no_echo)
                pre.axialize(no_echo)
                pre.copy(no_echo + "[2]",
                         no_echo.replace("_desc-vrA.nii.gz", "_eBase.nii.gz"))
                pre.volreg(no_echo, base=no_echo.replace("_desc-vrA.nii.gz",
                                                         "_eBase.nii.gz"),
                           interpolation='heptic',
                           onedfile=True, onedmat=no_echo.replace("vrA.nii.gz",
                                                                  "vrmat.aff12.1D"))
                pre.onedcat(no_echo.replace(".nii.gz", ".1D"),
                            os.path.join(fobj.dirname,
                                         "sub-{sub}_run-{run}_motion.1D".format(
                                             sub=str(fobj.get_entities()[
                                                         'subject']).zfill(2),
                                             run=str(fobj.get_entities()[
                                                         'run']).zfill(2))),
                            "[0..5]{1..$}")
            ...
            for fobj in pre.BIDS_layout.get(scope='derivatives',
                                            ...
                                            extension='.nii.gz', suffix='bold',
                                            run=run):
                pre.despike(fobj.path,
                            fobj.path.replace(".nii.gz", "_desc-pt.nii.gz"))
                pre.tshift(fobj.path.replace(".nii.gz", "_desc-pt.nii.gz"),
                           fobj.path.replace(".nii.gz", "_desc-ts.nii.gz"),
                           interp="heptic")
                pre.axialize(fobj.path.replace(".nii.gz", "_desc-ts.nii.gz"))
                pre.refit(fobj.path.replace(".nii.gz", "_desc-ts.nii.gz"),
                          deoblique=True, args="-TR 2.5")
            # print("Performing cortical reconstruction on %s" %sub_id)
            # preprocess.cortical_recon(bids_obj)

            if pre.is_verbose:
                print(
                    "Prepare T2* and S0 volumes for use in functional masking "
                    "and (optionally) anatomical-functional coregistration "
            ...
            for fobj in pre.BIDS_layout.get(scope='derivatives',
                ...
                try:
                    ...
                    echo = str(fobj.get_entities()['echo']).zfill(2)
                        ...
                        KeyError):  # handled in case of single echo BOLD data
                    echo = "00"
                pre.allineate(
                    fobj.path.replace(".nii.gz", "_desc-ts.nii.gz[2..22]"),
                    fobj.path.replace(".nii.gz",
                                      "_desc-e{s}_vrA.nii.gz".format(s=echo)),
                    mat=no_echo.replace("vrA.nii.gz", "vrmat.aff12.1D{2..22}"),
                    base=no_echo.replace("_desc-vrA.nii.gz", "_eBase.nii.gz"),
                    args="-final NN -NN -float")
                fobjs.append(fobj.path.replace(".nii.gz",
                                               "_desc-e{s}_vrA.nii.gz".format(
                                                   s=echo)))
            newname = os.path.join(fobj.dirname,
                                   "run-{r}_basestack.nii.gz".format(r=run))
            if len(fobjs) >= 2:  # multi echo
                pre.zcat(fobjs, newname)
                t2smap_workflow(newname,
                                echo_times)  # ,label="run-{r}_t2".format(r=run))
            ...
            if pre.is_verbose:
                print(
                    "--------Using AFNI align_epi_anat.py to drive anatomical-"
